<md-card ng-init="getData()" flex>
  <md-toolbar class="md-table-toolbar md-default">
    <div class="md-toolbar-tools">
      ...
      <span>Columnas en "@{{ params.table }}"</span>
      <div flex></div>
      <md-button class="md-icon-button" ng-click="add($event)">
        <md-tooltip md-direction="top">Crear Columna</md-tooltip>
        <md-icon>add</md-icon>
      </md-button>
  ...
  <md-table-container flex>
    <table md-table md-progress="promise">
      ...
      <tbody md-body>
        ...
        <tr md-row ng-repeat="data in data">
          <td md-cell nowrap>@{{ data.name }}</td>
          <td md-cell nowrap class="text-center">@{{ data.type }}</td>
          <td md-cell class="text-center"><span class="md-caption">@{{ data.default }}</span></td>
          <td md-cell nowrap class="text-center">
            <md-icon class="md-18">@{{ data.primary ? 'check_box' : 'check_box_outline_blank' }}</md-icon>
          </td>
          <td md-cell nowrap class="text-center">
            <md-icon class="md-18">@{{ data.unique ? 'check_box' : 'check_box_outline_blank' }}</md-icon>
          </td>
          <td md-cell nowrap class="text-center">
            <md-icon class="md-18">@{{ data.index ? 'check_box' : 'check_box_outline_blank' }}</md-icon>
          </td>
          <td md-cell nowrap class="text-center">
            <md-icon class="md-18">@{{ data.nullable ? 'check_box' : 'check_box_outline_blank' }}</md-icon>
          </td>
          <td md-cell nowrap class="text-center">
            <md-icon class="md-18">@{{ data.unsigned ? 'check_box' : 'check_box_outline_blank' }}</md-icon>
          </td>
          <td md-cell nowrap class="text-center">
            <md-icon class="md-18">@{{ data.autoincrement ? 'check_box' : 'check_box_outline_blank' }}</md-icon>
          </td>
          <td md-cell nowrap>
